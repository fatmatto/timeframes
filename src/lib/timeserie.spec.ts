...
test("Timeserie.lastAt() should return the last point with time <= the given", (t) => {
	const data: Point[] = [
		["2021-01-01", 1],
		["2021-01-02", 2],
		["2021-01-04", 4],
	...
	const ts = new TimeSerie("energy", data);
	t.is(ts.lastAt("2021-01-03")[1], 2);
	t.is(ts.lastAt("2021-01-06")[1], 5);
});
...
test("Timeserie.removeBetweenTime() should remove points from the timeserie", (t) => {
	...
	const ts = new TimeSerie("energy", data);
	const filtered = ts.removeBetweenTime(
...
test("Timeserie.dropNaN() should remove points from the timeserie", (t) => {
	const data: Point[] = [
		["2021-01-01T00:00:00.000Z", 1],
		["2021-01-01T00:00:00.000Z", Number.NaN],
...
test("Timeserie.fromIndex() should correctly create the series", (t) => {
	...
	t.is(
		true,
		ts.toArray().every((item: Point) => item[1] === 1),
	);
});
...
test("Timeserie.combine() should correctly combine the series", (t) => {
	...
	t.is(
		true,
		result.toArray().every((item: Point) => item[1] === 3),
	);
});
...
test("Timeserie.add() should correctly add the series to numbers and other series", (t) => {
	...
	t.is(
		true,
		ts3.toArray().every((item: Point) => item[1] === 10),
	);
});
...
test("Timeserie.sub() should correctly diff the series to numbers and other series", (t) => {
	...
	t.is(
		true,
		ts3.toArray().every((item: Point) => item[1] === -3),
	);
});
...
test("Timeserie.mul() should correctly multiply the series to numbers and other series", (t) => {
	...
	t.is(
		true,
		ts3.toArray().every((item: Point) => item[1] === 14),
	);
});
...
test("Timeserie.div() should correctly divide the series to numbers and other series", (t) => {
	...
	t.is(
		true,
		ts3.toArray().every((item: Point) => item[1] === 1),
	);
});
...
test("Timeserie.split() should split the timeserie", (t) => {
	...
	t.deepEqual(
		result.map((ts: TimeSerie) => ts.length()),
		[3, 3, 3, 1],
	);
});
...
test("TimeSerie.interpolate() should correctly avoid interpolating holes too big", (t) => {
	...
	const interpolated = reindexed.interpolate({
		method: "linear",
		limit: "44m",
	});
